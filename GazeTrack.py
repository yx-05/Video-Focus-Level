...
#-----------------------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------------------
FOCUS_LOG_FILE = "focus_intervals.csv"
focus_intervals = []
focus_start_time = None

program_start_time = datetime.now()
ten_minute_duration = timedelta(minutes=10.1666)

def save_focus_intervals(focus_intervals):
 
    if not focus_intervals:
        print("No focus intervals to save.")
        return

    with open(FOCUS_LOG_FILE, "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Start Time", "End Time", "Duration (seconds)"])
        for start, end in focus_intervals:
            duration = (end - start).total_seconds()
            writer.writerow([start.strftime("%Y-%m-%d %H:%M:%S"), end.strftime("%Y-%m-%d %H:%M:%S"), duration])
    print(f"Focus intervals saved to {FOCUS_LOG_FILE}.")
...
try:
    ...
    while True:
        ...
        is_focused = results.multi_face_landmarks is not None

        # Handle focus intervals based on the recording state
        if IS_RECORDING and is_focused:
            if focus_start_time is None:
                # User starts focusing
                focus_start_time = datetime.now()
        elif focus_start_time is not None:
            # User stops focusing
            focus_end_time = datetime.now()
            focus_intervals.append((focus_start_time, focus_end_time))
            focus_start_time = None
        ...
        if results.multi_face_landmarks:
            ...
            cv.circle(
                frame, mesh_points[LEFT_EYE_OUTER_CORNER][0], 3, (255, 255, 255), -1, cv.LINE_AA
            )  # Left eye left corner
            cv.circle(
                frame, mesh_points[RIGHT_EYE_INNER_CORNER][0], 3, (255, 255, 255), -1, cv.LINE_AA
            )  # Right eye right corner
            cv.circle(
                frame, mesh_points[RIGHT_EYE_OUTER_CORNER][0], 3, (255, 255, 255), -1, cv.LINE_AA
        ...
        # Writing the on screen data on the frame
            if SHOW_ON_SCREEN_DATA:
                ...
                cv.putText(frame, f"Blinks: {TOTAL_BLINKS}", (30, 80), cv.FONT_HERSHEY_COMPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                if ENABLE_HEAD_POSE:
                    cv.putText(frame, f"Pitch: {int(pitch)}", (30, 110), cv.FONT_HERSHEY_COMPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                    cv.putText(frame, f"Yaw: {int(yaw)}", (30, 140), cv.FONT_HERSHEY_COMPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                    cv.putText(frame, f"Roll: {int(roll)}", (30, 170), cv.FONT_HERSHEY_COMPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
        ...
        if datetime.now() - program_start_time >= ten_minute_duration:
            print("10 minutes elapsed. Exiting program.")
            if focus_start_time is not None:
                # Finalize ongoing interval before exiting
                focus_end_time = datetime.now()
                focus_intervals.append((focus_start_time, focus_end_time))
            break
...
"""
    ...
    # Save focus intervals to file
    save_focus_intervals(focus_intervals)
